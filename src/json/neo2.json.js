...
function rules() {
  ...
  const appleIdentifiers = [{ vendor_id: 1452 }, { is_built_in_keyboard: true }]
  ...
  const isAppleDevice = { type: 'device_if', identifiers: appleIdentifiers }
  const isNotAppleDevice = { type: 'device_unless', identifiers: appleIdentifiers }
  ...
  const isNotExcludedApp = { type: 'frontmost_application_unless', bundle_identifiers: ['com.apple.Terminal', 'org.gnu.Emacs', 'com.googlecode.iterm2', 'dev.warp.Warp-Stable'] }
  ...
  const appleGraveAccentAndTilde = function (condition) {
    return [].concat(
      eachKey({
        fromKeys: ['non_us_backslash'],
        fromModifiers: { optional: ['any'] },
        toPreKeys: [{ key_code: 'z', modifiers: ['left_option', 'left_shift'] }],
      ...
      eachKey({
        ...
        fromModifiers: { optional: ['any'] },
        toKeys: ['grave_accent_and_tilde'],
        conditions: condition === undefined ? [isLayoutActive] : [isLayoutActive, condition],
      })
    )
  }

  const windowsGraveAccentAndTilde = function (condition) {
    return eachKey({
  ...
  return [
    {
      ...
      manipulators: [].concat(
        ...
        neo2Modifiers('backslash', 'grave_accent_and_tilde', isAppleDevice),
        neo2Layer4(isAppleDevice),
        appleGraveAccentAndTilde(isAppleDevice)
      ),
    },
    {
    ...
    },
    {
      ...
      manipulators: [].concat(
        // --- Comment to prevent line combination by Prettier ---
        neo2Modifiers('non_us_pound', 'non_us_backslash', isNotAppleDevice),
        neo2Layer4(),
        windowsGraveAccentAndTilde()
      ),
    },
    {
